...
/**
 * 解析 menu.config.ts 中【一级菜单】（menuConfig 数组的直接元素）的 label。
 *
 * 用括号深度区分层级：仅收集「对象深度为 1」的 label，跳过 children 数组内
 * （深度 ≥ 2）的子菜单 label。供「选择父级菜单」列表使用——父级只能是一级菜单，
 * 不应把孙级子菜单也列出来让人选。
 *
 * 字符串感知：忽略字符串字面量内的 { } [ ]，避免 label 值含这些字符时误判。
 */
export const parseTopLevelMenuLabels = (content: string): string[] => {
  // 1) 定位 menuConfig 数组的开始 [
  const decl = /\bmenuConfig\b\s*[:=]\s*/.exec(content);
  if (!decl) return [];
  const arrOpen = content.indexOf("[", decl.index);
  if (arrOpen === -1) return [];

  // 2) 在数组范围内扫描，brace 深度为 1 的 label 即一级菜单
  const labels: string[] = [];
  let brace = 0;   // { } 深度：一级菜单对象=1，children 内 ≥2
  let bracket = 1; // [ ] 深度：进入数组即 1，回到 0 表示数组结束
  let i = arrOpen + 1;
  let inStr: string | null = null;

  while (i < content.length && bracket > 0) {
    const ch = content[i];

    // 字符串感知：字符串字面量内的结构字符不算
    if (inStr) {
      if (ch === "\\") {
        i += 2; // 跳过转义字符
        continue;
    ...
    if (ch === "{") brace++;
      ...
      // 仅一级菜单对象（深度 1）内的 label 才收集
      const m = /^label\s*:\s*['"`]([^'"`]+)['"`]/.exec(
        content.slice(i, i + 64)
      );
      if (m) {
        labels.push(m[1]);
        i += m[0].length;
